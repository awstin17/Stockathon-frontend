...
export class TrendComponent {
  ...
  checkIsRealStock(searchOrAddToFavorites) {
    this._stockservice.getMonthlyData(this.abbreviation.ticker)
      .subscribe(
        (response: any) => {
          console.log(searchOrAddToFavorites)
          if (response["Error Message"]) { window.alert("This is not a real stock symbol. Try another") }
          else if (searchOrAddToFavorites === "addToFavorites") { console.log(searchOrAddToFavorites); this.addStock() }
          else if (searchOrAddToFavorites === "search") {
            this.i = 0;
            this.closingNumbers = [];
            this.data = null;
            this.data = response;
